...
class ebookToPDF:
    ...
    def __init__(self, root):
        ...
        self.captureSpeed = IntVar() #캡쳐 간격 (내부적으로는 1/10초 단위, 0-100)
        self.captureSpeedDisplay = StringVar() #UI 표시용 (초 단위)
        self.captureSpeed.set(20) #기본값 2초 (20 * 0.1s)
        self.captureSpeedDisplay.set("2.0s")
        ...
        ttk.Label(contents, text="캡쳐 간격(초)").grid(column=1, row=7, sticky=W)
        ttk.Label(contents, textvariable=self.captureSpeedDisplay,width=4).grid(column=2, row=7, sticky=(W, E))
        ttk.Scale(contents, orient=HORIZONTAL, length=100, from_=0, to=100, variable=self.captureSpeed, command=self.updateSpeed).grid(column=3, row=7, sticky=(W,E))
    ...
    def updateSpeed(self, *args):#Scale바 값 변경시 호출
        #내부 변수 정수화
        speed_int = round(self.captureSpeed.get(),0)
        self.captureSpeed.set(speed_int)
        #UI 표시용 변수 업데이트 (초 단위)
        self.captureSpeedDisplay.set(f"{speed_int/10.0:.1f}s")
    ...
    def captureCall(self, *args):
        ...
        capture\
            ...
            .setCaptureSpeed(self.captureSpeed.get() * 100)\
